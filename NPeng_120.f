     ...
     3   AGE(800),HEIGHT(800),SUBLOGLIK(800),NDD(800),NDORIG(800),
     4   AICBIC(9997,2),
...
! Arg list
        ...
        double precision, dimension(max_doses), intent(INOUT) :: SIG
        integer, intent(IN) :: NDRUG, NADD
        double precision, dimension(max_doses,max_RS_J),                ! f90 Line Continuation
     1    intent(INOUT) :: RS
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
        double precision, dimension(max_input_dim,max_doses,2), save :: !
     1    XIV, BOL
...
!  Poisson probs is returned in RPAR(k_prod_pr)
     ...
     5   , k_c2_base, k_c3_base, k_c4_base, k_c5_base, i_errmod
     6   , i_misval, i_skip_ig, i_is_log10, i_Nnormalobs
     7   , i_is_poisson_obs, i_Npoissonobs, i_dvode_reserved
      IMPLICIT REAL*8(A-H,O-Z)
...
!  Normal approx.
     ...
     1                ,PoissonProb
               ...
               if (ig.eq.1) then
                 write (*,*) JSUB,IG,I,J,"is P  ",YMEAN,YOBSERVED,
     1             ObsError(I,J), poissonprob, 10**RPAR(k_prod_pr)
               endif
              ENDIF
           ELSE
...
2346            FORMAT(//' A S.D. < 0 FOR JSUB,IG = ',I5,I6,'.     '/
     ...
     1           + ((YOBSERVED-YMEAN)/ObsError(I,J))**2
               if (ig.eq.1) then
                 write (*,*) JSUB,IG, I,J,"is N  ",YMEAN,YOBSERVED
     1             ,IPAR(i_errmod),RPAR(k_gamma),ObsError(I,J)
     2             , RPAR(k_sum_z_sq)
               endif
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
	...
	SUBROUTINE FUNC2(JSUB,IG,M,YPRED,NUMEQT,NBCOMP,
     ...
     3      ObsError,IPAR,RPAR,ERRFIL)
       USE npag_utils, only: maxnumeq, max_m_per_obs
     1   , max_ODE_params, max_doses, max_ODE_comps, max_RS_J
     2   , max_input_dim, k_dvode_reserved, k_p_end, k_jsub, k_ig
     3   , i_ig, i_jsub, i_dvode_reserved
      IMPLICIT REAL*8(A-H,O-Z)
      include "interface_0SHIFT.txt"
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
      ...
      double precision, save, dimension(100,max_ODE_comps) :: XSTORE
...
40      IF(KNT .LE. M) GO TO 45
	...
	SUBROUTINE IDCALCYY(JSUB,IG,NPP,NDIM,ESTML,TPRED,NUMT,YYPRED,
     1      NUMEQT,NOBSER,MF,NBCOMP,RTOL,ATOL,o
     2      TIMCOPY,SIGCOPY,RSCOPY,BSCOPY,
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
	...
	END
     ...
     2      TIM,SIG,RS,BS,INTLIST,IPAR,ObsError,RPAR,ERRFIL)
         use npag_utils, only : maxnumeq,max_m_per_obs,max_RS_J
     1    ,max_ODE_params, max_doses, max_ODE_comps, max_input_dim
     2    ,k_dvode_reserved, k_ig, k_jsub, k_p_end, i_jsub, i_ig
     3    ,i_dvode_reserved
      implicit none
      include "interface_0SHIFT.txt"
      ...
      integer KNT,KNS,ID,NTL,ISKIPBOL,ISTEADY,NSET,IKNS,ICONV
      integer I,J,III,KNSNEW,KNTM1,NDO,NN,ISAME
...
40    IF(KNT  .LE. NUMT) GO TO 45
	...
	END DO
     ...
     1   ,cp_lrcs_to_rpar,max_input_dim,maxnumeq,max_m_per_obs
...
!$omp ThreadPrivate(/OBSER/,/TOCALC/,/PARAMD/,/INPUT/)
      COMMON/ERR/ERRFIL
...
!$omp&CopyIn(/OBSER/,/TOCALC/,/PARAMD/,/INPUT/)
...
!$          GOTO 676
 ...
 1254       FORMAT('  TOTAL NO. OF NUM. INTEG. WARNINGS IS ',I20)
  ...
  700      WORK(IG)=PYJGX(JSUB,IG)*CORDEN(IG,NVAR+1)
          ...
          WORKK(IG) = 0.D0
          WORK(IG) = 0.D0
          write (*,*) "Cycling past (JSUB,IG) ==", JSUB, IG,
     1       NBadInDO800, "of", NInDO800
        endif
  800   CONTINUE
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
!$omp   End Do
...
14001 continue
 ...
 1181   FORMAT(/'***************** END OF THE npagdriv.f FILE **********
	...
	SUBROUTINE FILRED(NOBSER,YO,C0,C1,C2,C3,C4,C5,NUMEQT,
     1    TIMCOPY,SIGCOPY,RSCOPY,BSCOPY,INTLIST)
         use npag_utils, only: maxnumeq,max_m_per_obs
     1    ,max_input_dim,max_doses,max_RS_J
        IMPLICIT REAL*8(A-H,O-Z)
        integer NOBSER
     ...
     2    YOO(max_m_per_obs,MAXNUMEQ),BS(max_doses,max_input_dim)
        ...
        COMMON/ERR/ERRFIL
        character ERRFIL*20
!$omp Threadprivate(/OBSER/,/SUM2/)
!$omp Threadprivate(/CNST/,/CNST2/,/DESCR/)
 ...
 1200 CONTINUE
	...
	END
     ...
     4  INTLIST,IPAR,ObsError,RPAR,ERRFIL)
      USE npag_utils, only: makevec,maxnumeq,max_m_per_obs
  ...
  150 END DO
	...
	END
     ...
     3    ,max_input_dim,k_gamma,k_flat,i_is_poisson_obs,i_is_log10
        IMPLICIT REAL*8(A-H,O-Z)
        DIMENSION VEC(NTOTPAR),IRAN(max_ODE_params),PX(max_ODE_params),
     1 SIG(max_m_per_obs,MAXNUMEQ),YO(max_m_per_obs,NUMEQT),
     2 C0(NUMEQT),C1(NUMEQT),C2(NUMEQT),C3(NUMEQT),C4(NUMEQT),C5(NUMEQT)
     3 ,AB(max_pop_rand_varbs,2)
      COMMON SIG
	COMMON/TOCALC/IRAN,PX,NOFIX,NSUB,gamma,flat,AB
       integer  NDIM,MF
       real*8 RTOL
...
!$omp ThreadPrivate(/TOCALC/)
  ...
  140    CONTINUE
	...
	END
     ...
     3    INTLIST,IPAR,ObsError,RPAR)
      use npag_utils, only: maxnumeq,max_m_per_obs,max_ODE_params
     1  ,max_pop_rand_varbs,max_doses,max_ODE_comps,max_RS_J
     2  ,max_input_dim
      IMPLICIT REAL*8(A-H,O-Z)
        DIMENSION START(N),STEP(N),XMIN(N),YO(max_m_per_obs,NUMEQT),
     ...
     1   max_pop_rand_varbs+1) :: P
        ...
        real*8, dimension(max_ODE_params) :: PCOPY
        real*8, dimension(max_m_per_obs) :: TIMCOPY
        real*8, dimension(max_doses) :: SIGCOPY
        real*8, dimension(max_doses,max_RS_J) :: RSCOPY
        real*8, dimension(max_doses,max_input_dim) :: BSCOPY
...
1       P(I,NN)=START(I)
     ...
     2    NBCOMP,NDIM,MF,RTOL,ATOL,PCOPY,RSCOPY,BSCOPY,
     3    INTLIST,IPAR,ObsError,RPAR)
        Y(NN)=FN
        ICOUNT=ICOUNT+1

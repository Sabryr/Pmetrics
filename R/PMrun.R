...
.PMrun <- function(type, model, data, run,
  ...
  if (type == "NPAG") {
    ...
    if (is.null(prior)) {
      ...
      if (inherits(prior, "NPAG")) { priorString <- c(0, "prior.txt") }
      #prior is an NPdata object
      if (is.character(prior)) { priorString <- c(0, prior) }
 ...
 if (parallel) { prefix <- "p" } else { prefix <- "s" }
  ...
  if (!intern | !auto) {
    ...
    } else { PMscript[getNext(PMscript)] <- paste(c("mv ", "move ", "mv ")[OS], model, " ", c("inputs", "inputs", "inputs")[OS], sep = "") }
